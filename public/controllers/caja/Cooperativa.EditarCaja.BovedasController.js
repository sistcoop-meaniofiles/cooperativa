...
angular.module('mean.cooperativa').controller('Cooperativa.EditarCaja.BovedasController',
    function($scope, $state, $filter, caja, toastr, SGAgencia, SGBoveda, SGDialog ){
                    ...
                    $scope.view.caja.$addBovedaCaja(bovedaCaja).then(
                        function(response){
                            ...
                            if(angular.isDefined($scope.view.loaded.bovedasAsignadas)){
                                $scope.view.loaded.bovedasAsignadas.push({boveda: $scope.combo.selected.boveda});
                            } else {
        ...
        $scope.removeBoveda = function($index, item){
            ...
            SGDialog.confirm('Eliminar', '¿Estas seguro de desvincular la boveda para la caja?. Debes de asegurarte que no existe saldo en caja en la moneda de la boveda para continuar.', function() {
